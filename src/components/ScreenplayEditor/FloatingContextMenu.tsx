...
const FloatingContextMenu: React.FC<FloatingContextMenuProps> = ({ position, onMenuClick }) => {
  ...
  return (
    <div 
      className="floating-context-menu fixed z-50 bg-white dark:bg-gray-800 rounded-lg shadow-xl flex p-1"
      style={{ 
        top: `${position.top}px`, 
        left: `${position.left}px`,
        transform: 'translateY(-100%)',
        fontSize: '18.56px' // Decreased by 15% from 21.84px
      }}
    >
      {menuItems.map((item) => (
        <button
          ...
          title={item.label}
          style={{ padding: '12px' }} // Increased padding
        >
          ...
          {/* Tooltip */}
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap" style={{ fontSize: '15.91px' }}>
            {item.label}
